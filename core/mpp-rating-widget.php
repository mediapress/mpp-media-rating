			...
			<?php if( $rated_media ): ?>
				...
				<?php foreach( $rated_media as $id ): ?>
					...
					<div class='mpp-g mpp-item-list mpp-media-list mpp-<?php echo $instance["type"] ?>-list'>
						...
						<div class="<?php mpp_media_class( 'mpp-widget-item mpp-widget-'.$instance["type"].'-item '. mpp_get_grid_column_class( 1 ) );?>">

							<div class='mpp-item-entry mpp-media-entry mpp-photo-entry'>

								<a href="<?php mpp_media_permalink( $id ) ;?>" <?php mpp_media_html_attributes( array( 'class' => "mpp-item-thumbnail mpp-media-thumbnail mpp-".$instance['type']."-thumbnail", 'data-mpp-context' => 'widget' ) ); ?>>
									<img src="<?php mpp_media_src('thumbnail', $id ) ;?>" alt="<?php echo esc_attr( mpp_get_media_title( $id) );?> "/>
								</a>

							</div>

							<a href="<?php mpp_media_permalink( $id ) ;?>" <?php mpp_media_html_attributes( array( 'class' => "mpp-item-title mpp-media-title ", 'data-mpp-context' => 'widget' ) ); ?> ><?php mpp_media_title( $id ) ;?></a>
	...
	public function form( $instance ) {
		...
		<p>
			...
			<?php foreach ( mpp_rating_get_component_can_be_rated() as $key => $label ) : ?>
				...
				<label>
					...
					<input class="widefat" name="<?php echo $this->get_field_name( 'component' ); ?>" type="radio" value="<?php echo $key; ?>" <?php checked( $component, $key ); ?>/>
					<?php echo $label; ?>
		...
		<p>
			...
			<select name="<?php echo $this->get_field_name( 'interval' ); ?>">
				...
				<?php foreach ( mpp_rating_get_intervals() as $key => $label ) : ?>
		...
		<p>
			...
			<?php foreach ( mpp_rating_show_media_of() as $key => $label ): ?>

				<label>
					<input name="<?php echo $this->get_field_name( 'user_type' ); ?>" type="radio" value="<?php echo $key; ?>" <?php checked( $key, $user_type  ); ?>/>
					<?php echo $label; ?>
				</label>
